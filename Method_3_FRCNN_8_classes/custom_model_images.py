...
# model
MODEL_NAME = 'frcnn_8'  # change to whatever folder has the new graph

# Path to frozen detection graph. This is the actual model that is used for the object detection.
pb_path = MODEL_NAME + '/frozen_inference_graph.pb'

# List of the strings that is used to add correct label for each box.
label_path = os.path.join('training', 'object-detection_frcnn_8.pbtxt')  # our labels are in training/object-detection.pbkt

NUM_CLASSES = 8  # we only are using one class at the moment (mask at the time of edit)
...
with detection_graph.as_default():
    with tf.Session(graph=detection_graph) as sess:
        ...
        for image_path in img_path:
            ...
            plt.savefig("output/detection_output{}.png".format(i))  # create an outputs folder for the images to be saved
